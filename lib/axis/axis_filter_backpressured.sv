//------------------------------------------------------------------------------
// File:    axis_filter_backpressured.sv
//
...
module axis_filter_backpressured
    #(
   ...
   always_ff @(posedge clk)
     if (rst)
       ...
       if (in_pipe1_axis.tlast && in_pipe1_axis.tvalid && in_pipe1_axis.tready)
         ipg_state <= BETWEEN_PACKETS;
       else if (in_pipe1_axis.tvalid && in_pipe1_axis.tready)
         ipg_state <= IN_PACKET;
       else
         ipg_state <= BETWEEN_PACKETS;
     else
   ...
   //
   always_comb
     in_axis.tready = ~in_pipe1_axis.tvalid | in_pipe1_axis.tready;
   ...
   //
   always_comb
     in_pipe1_axis.tready = ~in_pipe2_axis.tvalid | in_pipe2_axis.tready;
   ...
   always_ff @(posedge clk)
     if (rst) begin
        ...
        case(filter_state)
          ...
          //
          IDLE: begin
             if (in_pipe1_axis.tvalid && in_pipe1_axis.tready &&  pass)
          ...
          //
          PASS_PKT: begin
             if (in_pipe2_axis.tlast && in_pipe2_axis.tvalid && in_pipe2_axis.tready) begin
                if (in_pipe1_axis.tvalid && pass) // Back-to-back pass packet case.
                  filter_state <= PASS_PKT;
                else if (in_pipe1_axis.tvalid && in_pipe2_axis.tready)    // Back-to-back discard packet case.
                  filter_state <= DISCARD_PKT;
          ...
          DISCARD_PKT: begin
             if (in_pipe2_axis.tlast && in_pipe2_axis.tvalid && in_pipe2_axis.tready) begin
                if (!enable)
                  filter_state <= DISABLED;
                else if (in_pipe1_axis.tvalid && pass && in_pipe2_axis.tready)
                  filter_state <= PASS_PKT;
                else if (in_pipe1_axis.tvalid && in_pipe2_axis.tready)
                  filter_state <= DISCARD_PKT;
                else
                  filter_state <= IDLE;
             end else
          ...
          DISABLED: begin
             if (ipg_state==BETWEEN_PACKETS) begin
                if (!enable)
                  filter_state <= DISABLED;
                else if (in_pipe1_axis.tvalid && pass && in_pipe1_axis.tready)
                  filter_state <= PASS_PKT;
                else if (in_pipe1_axis.tvalid && in_pipe1_axis.tready)
                  filter_state <= DISCARD_PKT;
                else
   ...
   wire in_pipe2_axis_tready;
   always_comb
     in_pipe2_axis.tready = in_pipe2_axis_tready;
   ...
   axis_fifo_i0
     (.clk(clk),
      ...
      .in_tvalid(push),
      .in_tready(in_pipe2_axis_tready),
